...
class KSFactory {
    ...
    static async createServer(eventHandler, MONGO_DB = 27017, REST_SERVER = 3090, WS_PORT = 3000, ASSET_STORE = 8081) {
...
class KServer {
    constructor(...args) {
        ...
        setTimeout(() => {
            ...
            sock.output.on('line', (kSockData) => this.channelEventHandler(`${kSockData}`, 'WS'));
            const rest = new KServerChannel('rest.txt', '../kitsune-rest-server', false, false);
            rest.output.on('line', (kSockData) => this.channelEventHandler(`${kSockData}`, 'REST'));
...
class KServerChannel {
    constructor(file, cmd, fullCmd = false, newWindow = false) {
        this.outputStr = '';
